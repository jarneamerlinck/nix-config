...

{
  ...
  virtualisation.oci-containers.containers."photos-photoprism" = {
    ...
    environmentFiles = [ "/run/secrets-for-users/photoprism/env" ];
    volumes = [
  ...
  virtualisation.oci-containers.containers."photos-mariadb" = {
    image = "mariadb:11";
    environmentFiles = [ "/run/secrets-for-users/photoprism/db" ];
    volumes = [ "/data/docker/photoprism/database:/var/lib/mysql:rw" ];
    cmd = [
  ...
  systemd.services."docker-photos-mariadb" = {
    ...
    after = [ "docker-network-photos_photos.service" ];
    requires = [ "docker-network-photos_photos.service" ];
