...

{
  ...
  virtualisation.oci-containers.containers."tandoor-db_recipes" = {
    ...
    environmentFiles = [ "/run/secrets-for-users/recipes/env" ];
    volumes = [ "/data/docker/tandoor/postgresql:/var/lib/postgresql/data:rw" ];
    log-driver = "journald";
    extraOptions = [ "--network-alias=db_recipes" "--network=tandoor_tandoor" ];
  };
  systemd.services."docker-tandoor-db_recipes" = {
    ...
    after = [ "docker-network-tandoor_tandoor.service" ];
    requires = [ "docker-network-tandoor_tandoor.service" ];
  ...
  virtualisation.oci-containers.containers."tandoor-web_recipes" = {
    ...
    dependsOn = [ "tandoor-db_recipes" ];
    log-driver = "journald";
    extraOptions =
      [ "--network-alias=web_recipes" "--network=tandoor_tandoor" ];
  };
